...
public class EffAddSimplePieChart extends Effect {
    static{
        Skript.registerEffect(EffAddSimplePieChart.class,
    ...
    protected void execute(Event event) {
        ...
        if(serviceId != null && chartId != null && value != null){
            ...
            if(m == null){
                ...
                boolean result = m.addSimplePie(chartId, exprValue, event);
                if (!result){
                    SkriptbStats.getInstance().getLogger().warning("[Skript-bStats] Could not send simple pie chart " +
                            "value to metric " +
